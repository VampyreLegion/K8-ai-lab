  ...
  script:
    - |
      ...
      curl -s -X POST "${GITLAB_URL}/api/v4/projects/${CI_PROJECT_ID}/variables" \
        -H "PRIVATE-TOKEN: ${GITLAB_TOKEN}" \
        -H "Content-Type: application/json" \
        -d "{\"key\":\"APP_SUBDOMAIN\",\"value\":\"${SUBDOMAIN}\",\"variable_type\":\"env_var\",\"protected\":false,\"masked\":false}" \
        -o /dev/null || \
      curl -s -X PUT "${GITLAB_URL}/api/v4/projects/${CI_PROJECT_ID}/variables/APP_SUBDOMAIN" \
        -H "PRIVATE-TOKEN: ${GITLAB_TOKEN}" \
        -H "Content-Type: application/json" \
        -d "{\"key\":\"APP_SUBDOMAIN\",\"value\":\"${SUBDOMAIN}\",\"variable_type\":\"env_var\",\"protected\":false,\"masked\":false}" \
        -o /dev/null
